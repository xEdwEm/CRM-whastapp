...
export function MessageThread({
  ...
  // Auto-scroll to bottom on new messages. Smooth only when messages
  // are appended to the thread already on screen — the initial batch
  // (and conversation switches) jump instantly so opening a thread
  // doesn't animate through the whole history. Reduced-motion users
  // always get the instant jump.
  const prevScrollConvRef = useRef<string | undefined>(undefined);
  const prevMessageCountRef = useRef(0);
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;
    const isAppend =
      prevScrollConvRef.current === conversationId &&
    ...
    el.scrollTo({
      top: el.scrollHeight,
      behavior: isAppend && !prefersReducedMotion ? "smooth" : "auto",
    });
